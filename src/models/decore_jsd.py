...
class DeCoReJSD(BaseModel):
    ...
    def _calculate_kl_divergence(self, ori_probs, hallucinating_probs):
        return F.kl_div(ori_probs.log(), hallucinating_probs, reduction="batchmean")

    def _calculate_jsd(self, ori_logits, hallucinated_logits):
        p = torch.softmax(ori_logits, dim=-1)
        q = torch.softmax(hallucinated_logits, dim=-1)
        m = 0.5 * (p + q)
        kl_pm = self._calculate_kl_divergence(p, m)
        kl_qm = self._calculate_kl_divergence(q, m)
    ...
    def lm_score(
        ...
        with torch.no_grad():
            ...
            for i in range(base_logits.shape[0]):
                entropies += [
                    self._calculate_jsd(base_logits[i, :], hallucinated_logits[i, :])
                ]
